...
class PlateDataAnalysis:
    ...
    def filter_plates(self, results):
        ...
        for result in results:
            ...
            # Substitui '0' por 'D' nas posições de letras (0, 1, 2, 4, 5)
            text = list(text)  # Converte para lista para manipulação individual dos caracteres
            for i in [0, 1, 2, 4, 5]:  # Posições das letras
                if text[i] == '0':
                    text[i] = 'D'  # Substitui o '0' por 'D'
            
            # Reconstrua a string com a modificação
            text = ''.join(text)
